...
def get_environment_state(event : dict):
    ...
    for obj in event.metadata['objects']:
        ...
        if obj['pickupable']:
            ...
            for key in obj['position']: # obj['axisAlignedBoundingBox']['center']:
    ...
    if action_pos is not None:
        if action_type == 'PickupObject':
            ...
            event = controller.step(action='PutObject',
                                    objectId=objectId)
            PICKUP['position'] = None
            # print('Put object - ', object_name, object_name.startswith('CounterTop'))
            if object_name.startswith('CounterTop'):
                ...
                # print('After', MOVABLE_POSITION_DICT)
                # for mov_pos in MOVABLE_POSITION_DICT:
                #     if picked_object['name'].startswith(mov_pos['objectName']):
                #         pos_indices = list(range(len(COUNTER_POSITIONS)))
                #         shuffle(pos_indices)
                #         # print(pos_indices)
                #         for pos_index in pos_indices:
                #             # print('Checking position', COUNTER_POSITIONS[pos_index]['objectName'])
                #             if COUNTER_POSITIONS[pos_index]['objectName'] is None:
                #                 mov_pos['counter_position'] = COUNTER_POSITIONS[pos_index]
                #                 mov_pos['counter_position']['objectName'] = picked_object['name']
                #                 event = controller.step(
                #                     action="PlaceObjectAtPoint",
                #                     objectId=get_object_id(event, picked_object['name'])[0],
                #                     position=mov_pos['counter_position']['position'],
                #                     rotation=mov_pos['rotation']
                #                 )
                #                 # print('Found new position at', mov_pos['counter_position']['position'])
                #                 break
                #         break
            elif object_name.startswith('Microwave'):
                # Cannot really place object somewhere else in the microwave due to environment constraints
                pass
                # objectId, obj = get_object_id(event, picked_object['name'])
                # # print(objectId, obj)
                # # obj['position']['z'] -= 0.05 # np.random.uniform(0.02, 0.08)
                # # print('z diff', obj['position']['z'])
                # # obj['position']['y'] += 0.10
                # event = controller.step(
                #     action="PlaceObjectAtPoint",
                #     objectId=objectId,
                #     position={'x': 1.0300235748291016, 'y': 1.786553144454956, 'z': -1.3143658638000488}
                # )
                # print(event)
                # for _ in range(4):
                #     event = controller.step(
                ...
                #     )
                # print(event)
                # # print(get_object_id(event, picked_object['name'])[1])
        elif action_type == 'MoveObject':
            print('Searching for new position')
            # print('Before', MOVABLE_POSITION_DICT)
            orig_pos = None
            for mov_pos in MOVABLE_POSITION_DICT:
                if object_name.startswith(mov_pos['objectName']):
                    orig_pos = mov_pos
                    ...
                    if not object_name.startswith(mov_pos['objectName']):
                        if mov_pos['counter_position'] is None:
                            ...
                            if abs(x - mov_pos['counter_position']['position']['x']) < MIN_DIST and \
                                ...
                                print('X', abs(x - mov_pos['counter_position']['position']['x']))
                                print('Z', abs(z - mov_pos['counter_position']['position']['z']))
            ...
            if not obj['isOpen']:
                event = controller.step(action='OpenObject',
                                        objectId=objectId,
                                        openness=1.0)
                                        # openness=np.random.uniform(0.85 if object_name.startswith('Microwave') else 0.5,
                                        #                            1.0)) # np.random.uniform(0.6, 1.0))
            else:
...
def generate_sequence(seed : int, output_folder : str, num_frames : int = 200, prefix : str = ""):
    print('-> Seed:', seed)
    attempts = 0
    success = False
...
if __name__ == '__main__':
    output_folder = 'data/ithor_simple_2_1/'
    num_frames = 100
    num_sequences = 201
    prefix = 'test'
    overwrite = True
    ...
    for seq_idx in range(201, 1+num_sequences):
